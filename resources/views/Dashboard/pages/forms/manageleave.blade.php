<x-adminheader title="Manage leave" />
...
<body style="background-color:#F4F6FF;">
    <div class="container py-5 px-5" style="background-color:#F4F6FF;margin:0;">
        <div style="display: flex; justify-content: space-between; width: 100%">
            <a style="all: unset;" href="{{ route('add_leave_view') }}">
            ...
            <div style="display: flex; gap: 10px;">
                <a style="all: unset;" href="{{ route('leaves.export') }}">
                    <button type="submit" class="btn w-100 mr-2 my-3"
                        style="background-color: #4B49AC; color: white; border: none; padding: 10px 20px; font-size: 16px; border-radius: 5px; cursor: pointer;">
                        Download CSV File
                    </button>
                </a>
                <a style="all: unset;" href="{{ route('leaves.export.excel') }}">
                    <button type="submit" class="btn w-100 mr-2 my-3"
                        style="background-color: #28A745; color: white; border: none; padding: 10px 20px; font-size: 16px; border-radius: 5px; cursor: pointer;">
                        Download Excel File
                    </button>
                </a>
            </div>
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            $('table').on('click', '.delete-leave', function() {
                const leaveId = $(this).data('id'); // Get user ID from data attribute

                // Confirm if user wants to delete
                if (leaveId) {

                    $.ajax({
                        url: `{{ url('leaves/delete') }}/${leaveId}`, // Delete URL
                        method: 'DELETE',
                        data: {
            ...
            $('table').on('click', '.edit-leave', function() {
                const leaveId = $(this).data('id'); // Get user ID
                if (leaveId) {
                    window.location.href = `{{ url('addleave') }}/${leaveId}/edit`;
                }
            });
            ...
            $('table').on('click', '.btn-update', function() {
                ...
                currentRow.find('td').each(function(index) {
                    if (editablecolumns.includes(index)) {
                        ...
                        if (index === 1) updatedUserData.start_date = inputValue;
                        if (index === 2) updatedUserData.start_day_type = inputValue;
                        if (index === 2) updatedUserData.end_date = inputValue;
                        if (index === 2) updatedUserData.end_day_type = inputValue;
                        if (index === 2) updatedUserData.reason = inputValue;
                        if (index === 2) updatedUserData.approved_by = inputValue;
                        if (index === 2) updatedUserData.status = inputValue;
