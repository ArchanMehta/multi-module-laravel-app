<x-adminheader title="Manage Role" />
...
<body style="background-color:#F4F6FF;">
    <div class="container py-5 px-5" style="background-color:#F4F6FF;margin:0;">
        <a style="all: unset;" href="{{route('add_role_view')}}" >
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            $('table').on('click', '.delete-role', function() {
                const roleId = $(this).data('id'); // Get user ID from data attribute

                // Confirm if user wants to delete
                if (roleId) {
                    
                        $.ajax({
                            url: `{{ url('roles/delete') }}/${roleId}`, // Delete URL
                            method: 'DELETE',
                            data: {
            ...
            $('table').on('click', '.edit-role', function() {
                const roleId = $(this).data('id'); // Get user ID
                if (roleId) {
                    window.location.href = `{{ url('addrole') }}/${roleId}/edit`;
                }
            });
