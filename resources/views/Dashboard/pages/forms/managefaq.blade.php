...
<body style="background-color:#F4F6FF;">
    <div class="container py-5 px-5" style="background-color:#F4F6FF;margin:0;">
        <a style="all: unset;" href="{{route('add_faq_view')}}" >
            <button type="submit" class="btn w-25 mr-2 my-3" style="background-color: #4B49AC; color: white; border: none; padding: 10px 20px; font-size: 16px; border-radius: 5px; cursor: pointer;">
              Add FAQ
            </button>
          </a>
        ...
        <div class="card shadow-lg" style="display:flex">
            <div style="background: #4B49AC; color: #ffffff; font-weight: 600; font-size: 1.4rem; text-align: center; margin: 0; border-radius: 4px;">
        ...
        $(document).ready(function() {
            const table = $('.datatable').DataTable({
                ...
                columns: [{
                    ...
                    {
                        data: 'question', // Name column
                        name: 'question'
                    },
                    {
                        data: 'description', // Email column
                        name: 'description'
                    },
                    {
            ...
            $('table').on('click', '.delete-faq', function() {
                const faqId = $(this).data('id'); // Get user ID from data attribute

                // Confirm if user wants to delete
                if (faqId) {
                    
                        $.ajax({
                            url: `{{ url('faqs/delete') }}/${faqId}`, // Delete URL
                            method: 'DELETE',
                            data: {
            ...
            $('table').on('click', '.edit-faq', function() {
                const faqId = $(this).data('id'); // Get user ID
                if (faqId) {
                    window.location.href = `{{ url('addfaq') }}/${faqId}/edit`;
                }
            });
